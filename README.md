...
- S – SRP – Single responsibility principle - jedna odpowiedzialność czyli jeden powód do zmiany, np. Person nie powinien zmieniać adresów, lub zajmować się walidacją emaili
  klasa powinna mieć tylko jedną przyczynę zmian, na poziomie metody powinno się wykonywac tylko 1 zadanie, ogólnie przyczyną zmian jest aktor(grupa osób), moduł to spójny zbiór reguł i metod, które zmieniają się w tym samym czasie, każdy moduł powinien odpowiadać tylko przed 1 aktorem, zmiany powinny się wykonywać w tym samym czasie i z tego samego powodu, a jeśli nie to wynosimy je do np. do innej klasy, ale nie należy nadmiarowo (na siłę) wynosić zmian, należy klasy z wysoką kohezją, 1 przyczyną zmian, dzięki stosowaniu tej zasady unikamy pojawiania się niespodziewanych problemów oraz klasy mają wysoką kohezję, niski coupling, CQRS - oddzielenie uruchamiania konmend od query(odczytywania danych np. odczytanie/prezentacja raportu) 
- O – OCP – Open/Closed principle - klasa powinna być „otwarta na rozszerzenia a zamknięta na modyfikacje”. Uściślając – nie powinno dojść do sytuacji w której trzeba modyfikować kod. Jest to zabronione, gdyż zmiana deklaracji metody może spowodować błędne działanie w innych miejscach systemu. Np. tworząc klasych pochode Shape typu Circle, Square powinny implementować interfejs zawierający nazwę metody obliczającej powierzhnie. Czyli każda klasa pochodna posiada włąsną logikę obliczania pola i je nie zmieniamy np. do dając do systemu klasę Triangle, należy korzystać z interfejsów to bardzo ułatwia rozszeżanie funkcjonalnoścci, można też używać dziedziczenia lub kompozycji, ale ostrożnie interfejsy mogą komplikować kod, 
            korzystamy z ustawiania wartości stałych dla korzystania z jednej wartości w wielu miejscach
- L – LSP – Liskov substitution principle - w miejscu klasy bazowej możemy użyć dowolnej klasy, która po niej dziedziczy. Oznacza to, że klasa pochodna musi zachować 100% interfejsu klasy bazowej (wszystkie metody muszą przyjmować te same argumenty i zwracać te same typy). I np. w miejscu użycia klasy Figura moża podstawiać Kwadrat i Prostokąt i obliczać ich pole, definiujemy przejrzystą abstrakcję tak aby klasy pochodne mogły być podmieniane z klasami bazowymi, klasa pochodna powinna móc uzywać wszystkie metody interfejsu ewentualnie je rozszeżać, klasy powinny implementować jeden potrzebny interfejs
...
- D – DIP – Dependency inversion principle - moduły wysokopoziomowe nie powinny zależeć od modułów niskopoziomowych. Wszystkie zależności powinny w jak największym stopniu zależeć od abstrakcji, a nie od konkretnego typu. Czy np. w klasie PasswordReminder jako dbConnection nie powinniśmy używać konkretnego typu jak MySQlConnection ani PostgresSqlConnection tylko typ generyczny dbConnection. Nie tworzymy zależności cyklicznych.
